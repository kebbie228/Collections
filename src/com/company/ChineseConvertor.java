...
*/
 public static void main(String[] args) {
   int number = 100;
     ...
     int count = Integer.toString(number).length();// количествл цифр в числе number
   ...
   //System.out.println(Arrays.toString(digits));
     ...
     else if(hundreds>1) res=res.concat(hmap.get(hundreds)).concat(hmap.get(100));

     if(tens==1) res=res.concat(hmap.get(10));
     else if(tens>1) res=res.concat(hmap.get(tens)).concat(hmap.get(10));

     if(ones!=0) res=res.concat(hmap.get(ones));
